...
BASE_YEMOT_FOLDER = "ivr2:/85"  # שלוחה ראשית
...
def summarize_dvartorah_with_gemini(text_to_summarize: str, phone_number: str) -> str:
    ...
    # טעינת הוראות מגיטהאב
    instructions = ""
    if os.path.exists("gemini_instructions.txt"):
        with open("gemini_instructions.txt", "r", encoding="utf-8") as f:
            instructions = f.read()
...
def clean_text_for_tts(text: str) -> str:
    ...
    text = re.sub(r'[*•·]', '', text)  # הסרת כוכביות ונקודות
    text = re.sub(r'[A-Za-z]', '', text)  # הסרת אותיות לועזיות
    text = re.sub(r'\s{2,}', ' ', text).strip()  # ניקוי רווחים מיותרים
    return text
...
def upload_to_yemot(audio_path: str, yemot_full_path: str):
    ...
    with open(audio_path, "rb") as f:
        ...
        params = {"token": SYSTEM_TOKEN, "path": path_no_file, "file_name": file_name, "convertAudio": 1}
...
def upload_audio():
    """המשך שיחה (עם זיכרון ג'מיני)"""
    return handle_audio(memory_mode=True)


@app.route("/upload_audio_new", methods=["GET"])
def upload_audio_new():
    """התחלת שיחה חדשה (בלי זיכרון ג'מיני)"""
    return handle_audio(memory_mode=False)


def handle_audio(memory_mode=True):
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(suffix=".wav", delete=True) as temp_input:
            ...
            if memory_mode:
                ...
                final_dvartorah = summarize_dvartorah_with_gemini(recognized_text, phone_number + "_new")

            tts_path = synthesize_with_google_tts(final_dvartorah)

            # שלוחה אישית לכל מאזין
            personal_folder = f"{BASE_YEMOT_FOLDER}/{phone_number}"
            os.makedirs(personal_folder, exist_ok=True)
            unique_filename = f"dvartorah_{time.strftime('%Y%m%d_%H%M%S')}.wav"
            yemot_full_path = f"{personal_folder}/{unique_filename}"

            upload_success = upload_to_yemot(tts_path, yemot_full_path)
            os.remove(tts_path)

            if upload_success:
                playback_command = f"go_to_folder_and_play=/85/{phone_number},{unique_filename},0.go_to_folder=/000"
